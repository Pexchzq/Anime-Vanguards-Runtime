...
local VERSION = "AV-MACRO-BRAIN MAIN-SCRIPT V1.6"
...
local function canStartMacro(expectedMapName)
    local snapshot = scanOnce(false)
    local expectedOk = expectedMapName == nil
        or expectedMapName == ""
        or snapshot.mapName == expectedMapName

    return snapshot.canStartMacro and expectedOk, snapshot
end
