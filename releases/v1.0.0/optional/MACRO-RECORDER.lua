--[[
    ...
    VERSION: MACRO-RECORDER V1.3 - BRAIN-METADATA
...
local VERSION = "MACRO-RECORDER V1.3 - BRAIN-METADATA"
...
local function installHook()
    if unitEventHooked then
        return
    end
    unitEventHooked = true

    local myToken = recorderToken
    local oldNamecall
    oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if method == "FireServer" and self.Name == "UnitEvent" and recording then
            ...
            if action == "Render" then
                task.spawn(function()
                    if _G.MacroRecorderToken ~= myToken then
                        return
                    end
    ...
    log("UnitEvent.FireServer observer installed")
end
