--[[
    ...
    VERSION: MACRO-READER V0.2 - BRAIN-DRIVEN
...
local VERSION = "MACRO-READER V0.2 - BRAIN-DRIVEN"
...
local function waitForMacroStartReady(expectedMapName, requireEmptyUnits)
    ...
    while not stopRequested do
        ...
        local readyFromBrain = state.mapName == expectedMapName and state.endScreenVisible == false
...
local function executeStep(stepIndex, step)
    ...
    if step.action == "Upgrade" then
        ...
        if not waitForUpgradeConfirmation(runtime, step.upgradeLevel) then
...
local function macroReaderStartFromMapConfig(configPath)
    if running or waitingForStart then
        log("already running")
        return
    end
    ...
    log("PASS map config | waiting for start gate | mapName=" .. tostring(mapName) .. " | source=" .. tostring(source))
    ...
    task.spawn(function()
        ...
        if running then
            log("start gate passed but reader is already running")
            return
        end
...
local function macroReaderStatus()
    log(string.format(
        "running=%s | waitingForStart=%s | currentStep=%d | loadedSteps=%d",
        tostring(running),
